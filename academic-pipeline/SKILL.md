...
- Migrated from `academic-research-skills` v3.7.x for Codex use.
- Resolve paths relative to this skill folder. If a path starts with `academic-pipeline/`, strip that prefix. Shared resources referenced as `shared/...` are bundled inside this skill.
- Install sibling skills `deep-research`, `academic-paper`, and `academic-paper-reviewer` with this skill for full pipeline dispatch.
- Reference adapters are preserved under `scripts/adapters/`; they read local files and do not use Zotero Web API or other network services.
- This migration does not install Claude plugin hooks, slash commands, or automatic startup scripts.
- Original source: https://github.com/Imbad0202/academic-research-skills. License and notices are preserved in `LICENSE` and `NOTICE.md`.
