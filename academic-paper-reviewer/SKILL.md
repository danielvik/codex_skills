...
- Resolve paths relative to this skill folder. If a path starts with `academic-paper-reviewer/`, strip that prefix. Shared resources referenced as `shared/...` are bundled inside this skill.
- Sibling skill paths such as `academic-paper/...` or `academic-pipeline/...` require installing the migrated sibling skills together.
